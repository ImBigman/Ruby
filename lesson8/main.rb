...
class Main
  ...
  def adjust_route
    ...
    when '1'
      ...
      @station_list.each.with_index(1) { |_station, index| puts "#{index} - #{station.name}" }
    ...
    when '2'
      p route.full_route.each.with_index(1) { |_station, index| puts "#{index} - #{station.name}" }
  ...
  def tack_carriage
    ...
    p 'Всего вагонов у поезда: ' "#{train.carriages_count}" ' список: '
    (train.carriages_pull.each.with_index(1) do |carriage, index|
       puts "#{index} - #{carriage.name} : #{carriage.kind}
     вместимостью #{carriage.pull}"
                                             end
    ).to_s
  end
  ...
  def station_explorer
    ...
    station.each_train do |train, index|
      puts "#{index + 1} -  #{train.kind} поезд #{train.number}
       с вагонами в количестве: #{train.carriages_pull.count}"
    end
  end
